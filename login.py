...
def login_user(driver: webdriver, phone: str) -> Union[bool, Exception]:
    print("INFO: prepare qr code name")
    qr_code_name = path.join(os.getcwd(), "qr_codes", "%s.png" % phone)
    ...
    try:
        ...
        while not qr_code_scanned:
            ...
            driver.find_element(By.XPATH, "/html").screenshot("qr_test.png")
            time.sleep(1)

            if not check_element(driver, By.CSS_SELECTOR, ".landing-wrapper"):
                qr_code_scanned = True

            if not check_element(
                driver, By.XPATH, '//*[@id="app"]/div[1]/div/div[2]/div[1]/div/div[2]'
            ):
                continue
            ...
            if check_element(
                ...
                qr_code_reload = driver.find_element(
                    By.XPATH,
                    '//*[@id="app"]/div[1]/div/div[2]/div[1]/div/div[2]/div/span/button',
                )
...
def login(phone: str, force: bool = False) -> Union[str, Exception]:
    try:
        if phone[0] == "0":
            return [
        ...
        profile_dir = path.join(os.getcwd(), "profile", phone)
        success = False
        ...
        while not success:
            ...
            [success, error] = login_user(driver, phone)
            if not success:
                driver.close()
